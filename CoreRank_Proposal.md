...
基于 Go 实现面向竞技游戏服务端的匹配与排行榜服务，提供 gRPC / RESTful 双协议接入；使用 Redis ZSet 与 Lua 脚本承载匹配池、全局榜、赛季榜、活动榜和候选玩家原子摘取，设计 MatchTicket / MatchResult 状态流转支持入队、取消、超时和匹配结果查询；通过 Redis-backed server registry 完成房间资源分配，并提供最小 TCP roomserver v1 演示 join/ready/leave 闭环；使用 MySQL 可选持久化玩家、匹配票据和匹配结果，并接入 Prometheus 指标、CI 和 Robot 压测脚本完成可复现验证。
```
...
| 模块 | 当前状态 | 说明 |
|---|---|---|
| 排行榜 | 已实现 | 支持更新玩家分数、查询 TopN、查询个人排名，RESTful 支持 `leaderboard_type` 区分赛季榜/活动榜 |
