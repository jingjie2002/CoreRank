...
CoreRank V1+ 在 RESTful 排行榜入口中加入了 `leaderboard_type`，默认值仍为 `global`。例如：
...
服务端会把 `season:ss25` 映射到独立 Redis ZSet `{rank:season:ss25}`。这样可以用同一套排行榜服务承接赛季积分榜、活动榜、小游戏榜等场景，同时不改变原 gRPC v1 协议，也不影响全局榜的 MySQL 快照链路。
...
- 支持全局榜、赛季榜、活动榜等轻量排行榜维度，能对应赛季积分和运营活动 TopN 场景。
